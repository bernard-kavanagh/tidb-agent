...
PAIN: REUSABLE AGENT INFRASTRUCTURE (DOMAIN ADAPTER PATTERN)
- Companies building agent orchestration platforms designed to work across multiple domains or industries (not single-purpose agents)
- Companies whose agent framework is domain-agnostic — the orchestration logic, memory layer, and tool interface are generic; only the domain-specific data schema changes per deployment
- Companies building 'crews', 'swarms', 'teams', or 'fleets' of agents that share infrastructure but serve different use cases
- This signals they need a generic context plane that works across domains — the exact architecture where TiDB's unified substrate shines. The data plane changes per customer; the context plane (memory lifecycle, session state, fleet knowledge) stays identical.
...
NOTE ON 9 vs 10: Score 10 (not 9) when the company matches TWO OR MORE of: (a) agent orchestration platform processing millions of workflows, (b) multi-agent architecture with concurrent isolation needs, (c) explicitly building persistent agent memory or knowledge systems, (d) Fortune 500 customer base or enterprise-scale deployment. A company like CrewAI or LangChain that checks three of these boxes is a 10, not a 9. Reserve 9 for companies that match one profile perfectly. Reserve 10 for companies that are the ARCHETYPE — the customer you would put on stage at a conference.
